...
def segmentation_pipeline(lr=1e-3, batch_size=8, epochs=50):
    # 获取训练数据和ground-truth掩膜
    train_data_paths, train_mask_paths = data.load_seg_data_paths(config["data_root"]["segmentation"]["train"])
    test_data_paths, test_mask_paths = data.load_seg_data_paths(config["data_root"]["segmentation"]["test"])

    train_dataset = ImageSegDataset(train_data_paths, train_mask_paths, data.image_resize)
    test_dataset = ImageSegDataset(test_data_paths, test_mask_paths, data.image_resize)

    train_loader = DataLoader(train_dataset, batch_size=config["segmentation"]["batch_size"],
                              shuffle=True, pin_memory=False)
    test_loader = DataLoader(test_dataset, batch_size=config["segmentation"]["batch_size"],
    ...
    for epoch in tqdm(range(epochs)):
        epoch_loss = seg_train_epoch(net, train_loader, device, criterion, optimizer)
